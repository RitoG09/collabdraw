...
export default function isPointInShape(
  shape: ShapeType,
  point: DimentionType,
  ctx?: CanvasRenderingContext2D
) {
...
function isPointInRect(
  ...
  const [start, end] = shape.dimention;
...
function isPointNearArrow(
  ...
  const nearShaft =
    pointLineDist(point.x, point.y, shaft.x1, shaft.y1, shaft.x2, shaft.y2) <=
    tolerance;
  const nearLeft =
    pointLineDist(
      point.x,
...
function isPointNearLine(
  ...
  return (
    pointLineDist(point.x, point.y, shape.x1, shape.y1, shape.x2, shape.y2) <=
    tolerance
  );
}
...
function isPointOnDrawing(
  ...
  if (points.length === 1) {
    const d = Math.sqrt(
      (cords.x - points[0]!.x) ** 2 + (cords.y - points[0]!.y) ** 2
    );
