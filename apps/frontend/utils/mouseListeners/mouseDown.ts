...
export function getShapeIndexOnPrecisePoint(
  ...
  return shapes.findIndex((shape) => isPointInShape(shape, cords, ctx));
}

export function checkIsCursorInShape(
  cords: DimentionType,
  shapes: ShapeType[],
  ...
  if (index !== -1) {
    ...
    const shapeStart = shape!.dimention[0]!;
    setSelectedShapeIndex(index);
    setDragOffset({
...
export function checkIsCursorOnHandlers(
  cords: DimentionType,
