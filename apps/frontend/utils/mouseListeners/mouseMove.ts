...
export function handleMouseMovementOnMove(
  mouse: DimentionType,
  ...
  setShapes((prev: ShapeType[]) =>
    prev.map((shape, index) => {
      if (index === selectedShapeIndex) {
        ...
        const dim0 = shape.dimention[0]!;
        const dim1 = shape.dimention[1]!;

        const rel = {
          x: dim1.x - dim0.x,
          y: dim1.y - dim0.y,
        };
        const start: DimentionType = {
          x: mouse.x - dragOffset!.dx,
          y: mouse.y - dragOffset!.dy,
        };
        const end: DimentionType = {
          x: start.x + rel.x,
          y: start.y + rel.y,
        };
...
export function textResize(
  ...
  if (initialShape.type === "text") {
    ...
    const fontFamily =
      (initialShape.font || "24px Arial").split(" ").slice(1).join(" ") ||
      "Arial";
...
export function freeDrawResize(
  initialShape: ShapeType,
  mouse: DimentionType,
  shape: ShapeType,
...
export function handleMouseMovementOnResize(
  ...
  setShapes((prev: ShapeType[]) =>
    prev.map((shape, index) => {
      ...
      const [start, end] = initialShape.dimention;
      let newStart: any = { ...start };
      let newEnd: any = { ...end };
